        ...
        <div className="relative">
          <div className="flex overflow-x-auto pb-12 gap-6 snap-x no-scrollbar">
            {screenshots.map((screen, index) => (
              ...
              >
                <div className="relative aspect-[9/15] bg-slate-800 rounded-[2.5rem] p-3 border border-slate-700 shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
                  <div className="overflow-hidden rounded-[2rem] w-full h-full bg-slate-700 shadow-inner">
                    <img
                      src={screen.src}
                      alt={screen.title}
                  ...
                  <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-white px-5 py-2.5 rounded-full shadow-xl border border-slate-100 whitespace-nowrap z-10">
                    <p className="text-xs font-black text-slate-900 uppercase tracking-widest">{screen.title}</p>
                  </div>
                </div>
              </motion.div>
            ))}
